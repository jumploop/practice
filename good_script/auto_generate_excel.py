...
def get_datas(sql):
    # 一个传入sql导出数据的函数
    # 跟数据库建立连接
    conn = pms.connect(host='实例地址', user='用户', passwd='密码', database='库名', port=3306, charset='utf8')
...
def get_fields(sql):
    # 一个传入sql导出字段的函数
    conn = pms.connect(host='数据库实例地址', user='账号', passwd='密码', database='库名', post=3306, charset='utf8')
    cur = conn.cursor()
    cur.execute(sql)
    # 获取所需要的字段名称
...
def get_excel(data, field, file):
    # 将数据和字段名写入Excel的函数
    # 新建一个工作簿对象
    new = openpyxl.workbook.workbook.Workbook()
    # 激活一个新的sheet
    sheet = new.active
    # 给sheet命名
    sheet.title = '数据展示'
    # 将字段名循环写入Excel第一行，因为字段格式列表里包含列表，每个列表的第一元素才是字段的名称
...
def main():
    ...
    my_sql = sql = "SELECT a.id '用户ID',a.gmtCreate '用户注册时间',af.lastLoginTime '最后登录时间',af.totalBuyCount '历史付款子单数',af.paidmountUSD '历史付款金额',af.lastPayTime '用户最后支付时间'FROM table a\
