...
def getWeather(city_code):
    ...
    return {'日期':datetime.datetime.now().strftime("%Y-%m-%d"),'温度': '{}/{}'.format(temperatureHigh,temperatureLow), '天气': weather, '风力': wind.i.string,
...
def sendWeatherMsg(msg):
    try:

        # 你朋友的微信名称，不是备注，也不是微信帐号。
        my_friend = bot.friends().search('Fighting')[0]
        my_friend.send(msg)
        # 每86400秒（1天），发送1次
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(current_time)
        time.sleep(2)
        if start_time in str(current_time):
            dic = getWeather(101110300)
            msg = strDic(dic)
            sendWeatherMsg(msg)
